...
exports.addAchievements = async (req, res) => {
  try {
    ...
    const achievementFile = req.files?.image ? req.files.image[0] : null;
    let imageUrl = null;
    ...
    if (achievementFile) {
      ...
      imageUrl = saveFile(achievementFile, path.join(commonPath, pathAchievement), fileName);
    }
  ...
  try {
    ...
    if (achievementFile) {
      ...
      if (achievement.imageUrl) {
        const oldImagePath = path.join(baseDir, achievement.imageUrl.replace("files/", ""));
...
exports.deleteAchievements = async (req, res) => {
  try {
    ...
    if (achievement.imageUrl) {
      const imagePath = path.join(baseDir, achievement.imageUrl.replace("files/", ""));
